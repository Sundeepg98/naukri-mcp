...
async def api_get(path: str, params: dict = None, _attempt: int = 0) -> dict:
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get(url, headers=headers) as resp:
            ...
            if resp.status == 401 and _attempt == 0:
                ...
                return await api_get(path, params, _attempt=1)
            if resp.status in RETRIABLE_STATUSES and _attempt < MAX_RETRIES:
                ...
                return await api_get(path, params, _attempt=_attempt + 1)

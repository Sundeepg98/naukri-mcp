"""Deep tests for naukri_server.tools.assessments — listing, completeness, routing.

Every test is PURE: no network, no browser, no file I/O.
Recovered from deleted tier24_assessments.py.
"""
...
def _make_assessment(**overrides) -> dict:
    base = {
        ...
        "results": {"score": 85, "rank": 120, "accuracy": 90.0, "status": "PASS"},
        "maxMarks": 100, "passingMarks": 60, "testId": "T001",
    }
...
def _make_dashboard_response(assessments=None, extras=None) -> dict:
    board = {
        "pc": 78, "totalSearchAppearancesCount": 200, "profileViewCount": 15,
        "isPaidUser": False, "isPremium": True, "isAIResumeEligible": True,
...
class TestAssessmentsDispatch:
    ...
    async def test_completeness_action_routes(self, mock_comp):
        mock_comp.return_value = {"status": "success", "completeness_percent": 90}
        from naukri_server.tools.assessments import _assessments_dispatch

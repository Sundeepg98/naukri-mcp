...
class TestScreeningQuestionRetry:
    ...
    @patch("naukri_server.database.get_application", new_callable=AsyncMock)
    async def test_retry_needs_input_with_answers(self, mock_get_app, mock_apply):
        """When status=needs_input and answers provided, allow retry."""
        mock_get_app.return_value = {"job_id": "110326018660", "status": "needs_input", "pending_questions": 2}
    ...
    @patch("naukri_server.tools.apply._apply_single", new_callable=AsyncMock)
    @patch("naukri_server.database.get_application", new_callable=AsyncMock)
    async def test_block_needs_input_without_answers(self, mock_get_app, mock_apply):
        """When status=needs_input but NO answers, still block."""
        mock_get_app.return_value = {"job_id": "110326018660", "status": "needs_input"}
        from naukri_server.tools.apply import naukri_apply
    ...
    @patch("naukri_server.tools.apply._apply_single", new_callable=AsyncMock)
    @patch("naukri_server.database.get_application", new_callable=AsyncMock)
    async def test_block_already_applied_with_answers(self, mock_get_app, mock_apply):
        """When status=applied, block even with answers."""
        mock_get_app.return_value = {"job_id": "110326018660", "status": "applied"}
        from naukri_server.tools.apply import naukri_apply
        result = await naukri_apply(job_id="110326018660", answers={"q1": "yes"})
        assert result["status"] == "already_applied"
        mock_apply.assert_not_called()
    ...
    @patch("naukri_server.tools.apply._apply_single", new_callable=AsyncMock)
    @patch("naukri_server.database.get_application", new_callable=AsyncMock)
    async def test_new_job_no_existing_record(self, mock_get_app, mock_apply):
    ...
    @patch("naukri_server.tools.apply._apply_single", new_callable=AsyncMock)
    @patch("naukri_server.database.get_application", new_callable=AsyncMock)
    async def test_block_error_status_without_answers(self, mock_get_app, mock_apply):
        """When status=error but no answers, block."""
        mock_get_app.return_value = {"job_id": "110326018660", "status": "error"}
        from naukri_server.tools.apply import naukri_apply
        result = await naukri_apply(job_id="110326018660")
        assert result["status"] == "already_applied"
        mock_apply.assert_not_called()
